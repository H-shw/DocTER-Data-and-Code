...
def compute_z(
    ...
    target_ids = tok(request["target_new"]["str"], return_tensors="pt").to(f"cuda")[
        "input_ids"
    ][0]
    ...
    for i in range(len(rewriting_prompts)):
        ...
        filtered_ids = [id.item() for id in rewriting_targets[i] if id.item() != -100]
        # 使用tokenizer解码剩余的ID
        decoded_text = tok.decode(filtered_ids, skip_special_tokens=True)
        # print(decoded_text)
    # quit()
